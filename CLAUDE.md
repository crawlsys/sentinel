...
| `sentinel-application` | — | Use cases: engine, classifier, gate, 89 hook modules |
...
89 hook modules (one `.rs` file per hook in `hooks/`). The categories below are **representative, not exhaustive** — they show a sampling of each category, not all 89:
