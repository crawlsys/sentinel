...
- Consul ADR-017 (Artifact + Requirement Metadata Extensions) — defines the wire format (`ArtifactReference`, `RequirementRef`); this doc specifies the sentinel-side gate that enforces it
...
BA1 (every claim source-pinned) and BA3 (every recommendation traceable to a requirement) are the two S-tier disciplines that distinguish the AI-factory BA product from a generic LLM. Consul ADR-017 specifies the wire format that lets dispatcher and legatus exchange artifact and requirement metadata. **This doc specifies the sentinel-side enforcement** — three new hooks that turn the wire format into a hard gate at the hook layer:
...
All three are pure sentinel hooks; they do not require code changes in consul or in the BA-orchestrator. They read consul ADR-017's structured payloads (which flow through sentinel's existing PreToolUse / PostToolUse pipeline) and apply policy. The gates can be permissive (warn-only) early, then promoted to blocking once operator confidence in the connector layer is established.
...
Consul ADR-017 specifies that `RelayInstruction` and `InstructionResult` carry optional `artifacts: Vec<ArtifactReference>` and optional `requirement_refs: Vec<RequirementRef>` / `fulfilled_requirements: Vec<RequirementRef>`. The wire shape is right. But the wire format alone doesn't enforce that the fields are *populated correctly* — or at all.
...
**Action:** Reads the output's `artifacts: Vec<ArtifactReference>` (per consul ADR-017). For each cited `artifact_id`, validates:
...
**Failure modes:**
- Cited artifact was retrieved in a *prior* session and the lookback window has expired → the agent should re-retrieve. Hook surfaces "citation X is from session Y; please re-retrieve to refresh." Not a block; an instruction to refresh.
...
**Action:** Reads the output's `requirement_refs: Vec<RequirementRef>` (per consul ADR-017). For each:
1. **Matrix presence**: the `matrix_row_id` exists in the active orchestration's requirements matrix (which is stored as part of the orchestration's state per consul ADR-017's `orchestration_id`).
...
- Adapter in `sentinel-infrastructure/src/requirements/` — the matrix itself lives in the BA-orchestrator (per consul ADR-017 the orchestrator owns the matrix); the adapter is a read-only client.

**Failure modes:**
- Matrix not yet populated (early in an orchestration) → soft-warn rather than block; the workflow is still in discovery phase.
- Matrix lookup unavailable → fall back to "warn-only" mode; operator notified of the degraded enforcement; resume strict mode when matrix returns.
...
2. **Warn-mode**: the hook logs and surfaces findings to the operator via consul as runtime warnings, but still allows the action. Used while the BA-orchestrator is being trained/refined to populate citations and requirement refs reliably.
...
## 5. Interaction with consul-side ADR-017

Consul ADR-017 is the wire-format spec; this doc is the enforcement spec. The relationship is one-way:

- **ADR-017 (consul)**: defines `ArtifactReference`, `RequirementRef`, optional fields on `RelayInstruction` / `InstructionResult`, `RequestBriefing` / `BriefingResponse`. The shape of the data on the wire.
...
If consul ADR-017 doesn't ratify, sentinel's hooks will read empty `Vec<ArtifactReference>` from legacy messages and gate accordingly (which, in `observe-only` mode, means logging only; in `block-mode`, means blocking any BA output without citations, which is correct behavior even if the wire format isn't being populated).
...
- **The matrix-state storage**. The orchestration owns the matrix per consul ADR-017; sentinel reads via the `RequirementMatrixPort` adapter. The storage mechanism (in-memory, SQLite, etc.) is an adapter detail.
...
`requirements_traceability_gate` needs the orchestration's matrix. If the orchestration hasn't published the matrix to a reachable location, the gate degrades.

Solution: `RequirementMatrixPort` adapter has a `last_known_good` fallback. If the matrix is unreachable, gate runs against the most recent snapshot and emits a Warn-class finding about the staleness.
...
- **`provenance_validate` hook tests**: artifact-with-valid-provenance → allow; artifact-with-no-provenance → block; stale citation → warn (or block in catastrophic mode); operator override → allow with audit.
- **`requirements_traceability_gate` hook tests**: recommendation-with-traceable-requirement → allow; recommendation-without-requirement → block (coverage failure); requirement-hash-mismatch → warn; matrix empty → soft-warn.
...
2. **Per-tenant matrix isolation**. If multiple BA-orchestrators share a sentinel instance (multi-tenant), can they see each other's matrices? Same answer as BA6 §6.4 and BA5 §9.6: capability-token-scoped (consul ADR-018 territory).
...
- **Owner:** Gary Somerhalder ratifies. Co-requires consul ADR-017 ratification (wire format) and BA6 ratification (connector layer). Compatible with BA5 (which consumes these signals) and A3 (which runs in the same hook pipeline).
...
This doc relies on the upstream brief's evidence base for BA1 and BA3 (already cited there); no new external citations introduced. The consul ADR-017 design has its own caveats inherited from the brief.
...
- Treating consul ADR-017, BA6, BA5, A3, R5, R8 as the surrounding context that gives these hooks their meaning.
