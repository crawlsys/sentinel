...
async fn build_capability_route_graph_with_checkpointer(
    ...
    let builder = StateGraphBuilder::<CapabilityRouteState>::with_schema(schema.clone())
        ...
        .add_async_node_with_config(
            ...
            node_config(ROUTED, checkpointer_backend, checkpointer_scope),
        )
        .add_async_node_with_config(
            NO_ROUTE,
            |s: CapabilityRouteState| async move {
            ...
            node_config(NO_ROUTE, checkpointer_backend, checkpointer_scope),
        )
...
pub async fn run_capability_route_decision_report(
    ...
    let thread_id =
        crate::decision_graph_store::run_thread_id("capability_route", &state.identifier, &state)?;
