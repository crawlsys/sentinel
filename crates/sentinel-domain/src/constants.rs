...
/// How long the marketplace dependency-check cache (`dep_check.rs`) is considered fresh.
///
/// One day is long enough that the check rarely re-runs in normal use, short
/// enough that adding a new crate-level dep gets noticed within a working day.
pub const DEP_CHECK_CACHE_TTL: Duration = Duration::from_secs(24 * 60 * 60);

/// How recent a `plans/*.md` file must be to satisfy the resumed-session fallback.
///
/// Used in `tool_usage_gate`. One week balances "session resumed days later"
/// against "stale plan from a different feature still on disk".
pub const PLAN_FILE_FRESH_WINDOW: Duration = Duration::from_secs(7 * 24 * 60 * 60);

/// How old a session-events directory must be before cleanup removes it.
///
/// `channel_events::cleanup_stale_sessions` uses this during `SessionStart`.
...
/// Fully-loaded engineer cost per working day (USD).
///
/// Derived as `HUMAN_FULLY_LOADED_USD_PER_YEAR / 260` (52 weeks × 5 working days).
/// Stored as a constant rather than computed so the ROI rollup is deterministic
/// and reviewers don't have to retrace the arithmetic.
pub const HUMAN_FULLY_LOADED_USD_PER_DAY: f64 = 654.0;

/// Average mid-level developer velocity: days per story point.
///
/// Derived from "10 points per week = 0.5 days per point" — a commonly cited
/// team-velocity benchmark. Override per project if your team's actual velocity
/// differs materially.
