...
- **8 channel-event call sites updated to thread `ctx.fs` + `ctx.env`**: `build_notify` (2 emits), `context_monitor` (1), `mcp_health` (1), `plan_organizer` (1), `subagent_stop` (1), `task_completed` (1), `teammate_idle` (1), `session_init::cleanup_stale_sessions` (1). Each emits keep `input.session_id` / `input.cwd` resolution unchanged; only the `ctx.fs, ctx.env,` prefix is added. `_ctx` → `ctx` rename in 3 hooks (`build_notify`, `subagent_stop`, `task_completed`, `teammate_idle`) where the parameter was previously unused.
- After this batch: total prod-side IO leaks drop from 24 to 15 (−9). Workspace tests: 742 passing, 0 failing. Files with prod leaks: 6 (was 6, but `channel_events.rs` collapsed from 10 sites to 1).

- **PR3 — `ctx.fs` sweep across 4 hooks** (DDD/hex audit follow-up to PR1):
  - `subagent_start.rs` — `process()` reads active-skill state via `ctx.fs.home_dir()` + `ctx.fs.read_to_string()` instead of `dirs::home_dir()` + `std::fs::read_to_string`. Hook signature already had `ctx`; just renamed `_ctx` → `ctx`. Net 1 prod `std::fs::` + 1 prod `dirs::home_dir` deleted.
  - `tool_usage_gate.rs` — `pub fn detect_plan_mode_from_transcript` now takes `&dyn FileSystemPort` as its first parameter; the prod call site in `process()` passes `fs`. Tests gain a small `RealTestFs` adapter for the 13 sites that read real `tempfile`-backed transcripts. `MockFs::read_to_string` upgraded to pass-through to real disk when the path exists, so `process()` tests that build a transcript on disk get correct plan-mode detection. Net 1 prod `std::fs::` deleted.
  - `pre_push_steel_test.rs` — `state_file_path`, `has_recent_steel_test`, `has_recent_steel_test_pub`, `record_steel_test_passed`, `repo_has_steel_config_in`, `repo_has_steel_config` all thread `&dyn FileSystemPort`. The CLI's `steel_test_cmd.rs` constructs `RealFileSystem` and passes it to `record_steel_test_passed` and `has_recent_steel_test_pub`. Tests gain a `RealFsTest` adapter (real-disk `FileSystemPort` impl) for tempfile-backed scenarios; the two `process_post_tool` tests that need state to actually persist switch from `stub_ctx()` to a new `real_fs_ctx()` helper. Net 5 prod `std::fs::` + 2 prod `dirs::home_dir` deleted.
  - `phase_gate.rs` — five `dirs::home_dir().expect(...)` panic-on-None sites threaded through `&dyn FileSystemPort`: `extract_phase_file`, `check_bash_redirect_to_protected`, `check_blocked_bash_patterns` (added `fs` param to forward), `check_protected_path_write`, `check_protected_canonical`. The `process()` entry point already has `fs` in scope; ripples are mechanical. Panic-on-`None` semantics preserved for the security-critical paths (Attack #87/90/91/92/97 fixes — empty-PathBuf fallback would silently bypass the symlink-escape and protected-path checks). Net 5 prod `dirs::home_dir` deleted.
- After this batch: total prod-side IO leaks in `crates/sentinel-application/src/` drops from 39 to 24 (−15). Workspace tests: 742 passing, 0 failing. Files still leaking IO: 6 (was 16 pre-audit).
- **Remaining tail (deferred to PR4)**: `git_hygiene.rs` (3 sites — needs `FileSystemPort::canonicalize`), `doc_drift.rs` (2 — `acquire_lock` needs an OS file handle, no port equivalent), `channel_events.rs` (10 — public API rewrite, batch with EnvPort sites in same PR), `ntfy_push.rs` (3 — fire-and-forget helper, low priority), plus a handful of session/scanner/interceptor sites that need targeted treatment.
