...
| `~/.claude/sentinel/metrics/` | **RW** | Hook output. The container's sentinel writes JSONL here; the operator's host-side consumer tails the same inode. This is the ONLY RW host path the container has. |
...
The boundary is intentionally asymmetric: the container can
write to **only one** host path — the metrics directory. The
host can read everything the container produces; the container
can only deposit observability events into a known slot and
read its own RO mounts.
